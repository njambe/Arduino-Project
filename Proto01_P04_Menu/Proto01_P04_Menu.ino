...
void sendServer(char* tttt, int len) {
  LED_Y_ON;
  ether.sendUdp(tttt, len, IN_PORT, ether.hisip, OUT_PORT);
  LED_Y_OFF;
}
...
void loop(void) {
  ...
  CND_CENTER {
    LED_Y_ON;
//    sendServer(strcat("Option ", "test"), 11);
    LED_Y_OFF;
    WHILE_CENTER_PRESS; 
    delay(350);
  }
...
void screen1(void) {
  ...
  gotoXY(0,1);
  if (opt == 0) { LcdString(">"); }
  LcdString("Default IP");
  gotoXY(0,2);
  if (opt == 1) { LcdString(">"); }
  LcdString("Setup param");
  gotoXY(0,3);
  if (opt == 2) { LcdString(">"); }
  LcdString("DHCP");
}

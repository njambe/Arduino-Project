...
// ` replaces the ^ upside down when written on screen
...
// History variables
String history[HIST_LEN];
int historyLast = 0, offset = 0;
...
{  {    0x00, 0x00, 0x00, 0x00, 0x00    } // 20  
  ...
  ,{    0x19, 0x20, 0x40, 0x20, 0x19    } // 60 ` !! NEW DOWN
...
void inline ethernetInit(void) {
  if (ether.begin(sizeof Ethernet::buffer, mymac) == 0)
    LED_R_ON;
  #if STATIC
    ether.parseIp(ether.myip, MY_IP);
    ether.parseIp(ether.gwip, GW_IP);
...
void loop(void) {
  ...
  if (vw_get_message(buf, &buflen))      // We check if we have received data
  {
    ...
    LcdString("            ");
    gotoXY(0,2);
//    historyLast = (historyLast < HIST_LEN)? historyLast + 1 : 0;
    for (int i = 0; i < buflen; i++)
    {
//      history[historyLast] += buf[i];
//      message = true;
//      offset = 0;
      LcdCharacter(buf[i]);
    }
    #if DEBUG
      Serial.println(history[historyLast]);
    #endif
    LED_Y_OFF;
    LED_G_ON;
  }
...
//  CND_UP {
//    offset++;
//    displayMessages(historyLast + offset);
//    delay(250);
//  }
//  
//  CND_DOWN {
...
//  
//  if (message) {
//    displayMessages(historyLast);
//  }
}

void displayMessages(int beginning) {
  gotoXY(0,1);
    for (int i = 1; i < 5; i++) {
      gotoXY(0, i + 1);
      LcdString(history[(beginning - i >= 0)? beginning - i : HIST_LEN - i]);
    }
}

...
#define MY_IP "192.168.1.80"        // Not used
...
void sendServer(int code) {
  char data[22];
  int len_data = 0;
  
  switch(code) {
    case UP_PRESS:     strcpy(data, "Button UP pressed");     len_data = 17; break;
    case DOWN_PRESS:   strcpy(data, "Button DOWN pressed");   len_data = 19; break;
    case CENTER_PRESS: strcpy(data, "Button CENTER pressed"); len_data = 21; break;
    case LEFT_PRESS:   strcpy(data, "Button LEFT pressed");   len_data = 19; break;
    case RIGHT_PRESS:  strcpy(data, "Button RIGHT pressed");  len_data = 20; break;
    default:           strcpy(data, "Unknown code");          len_data = 12; break;
  }
...
void setup(void) {  
  ...
  gotoXY(0,2);
  LcdString("Server IP:");
  // This display works for IP addresses like 192.168.1.x
  gotoXY(0,3);
  LcdString(itoa(ether.hisip[0], buf, 10));
  LcdString(".");
  gotoXY(24,3);
  LcdString(itoa(ether.hisip[1], buf, 10));
  LcdString(".");
  gotoXY(48,3);
  LcdString(itoa(ether.hisip[2], buf, 10));
  LcdString(".");
  gotoXY(58,3);
  LcdString(itoa(ether.hisip[3], buf, 10));
